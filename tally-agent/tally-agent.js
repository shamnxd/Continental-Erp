...
// Configurations (Can be adjusted via Environment Variables)
const ERP_WS_URL = process.env.ERP_WS_URL || "ws://localhost:5000/tally-sync";
const TALLY_SYNC_TOKEN = process.env.TALLY_SYNC_TOKEN || "tally_secret_token_123";
const TALLY_HTTP_URL = process.env.TALLY_HTTP_URL || "http://localhost:9000";
...
function connect() {
  console.log(`🔌 Connecting to Continental Cloud ERP at: ${ERP_WS_URL}...`);
  
  ws = new WebSocket(`${ERP_WS_URL}?token=${TALLY_SYNC_TOKEN}`);
...
async function handleEntitySync(data) {
  ...
  try {
    if (entityType === "Client") {
      ...
      // Pre-create 'Purchase Accounts' ledger
      ...
      // Pre-create 'CGST' and 'SGST' tax ledgers if GST amount is present
      ...
      // 3. Create Sales Voucher (Invoice)
    ...
    // Parse Tally response envelopes for success/failure
    const envelope = parsedResponse.ENVELOPE;
    
    // Tally returns statistics indicating CREATED, ALTERED, or ERRORS
    let isSuccess = false;
    let errorMessage = "";
    let tallyVoucherNo = "";
...
async function postToTally(xmlContent) {
  const res = await axios.post(TALLY_HTTP_URL, xmlContent, {
...
function formatDate(dateInput) {
  ...
  // OVERRIDE FOR TALLY EDUCATIONAL MODE dates compatibility (Only accepts 1st, 2nd, 31st)
  // If date is not 1st, 2nd, or 31st, we default to the 1st of that month.
  let day = d.getDate();
...
function extractTallyExpenseLedgers(parsed) {
  ...
  for (let i = 0; i < bsNameArray.length; i++) {
    ...
    if (nameItem && nameItem.DSPACCNAME && nameItem.DSPACCNAME.DSPDISPNAME) {
      ...
      if (name.toLowerCase() === "sales") continue; // Skip sales ledger
...
// Dynamic pull for invoices (Sales Vouchers)
...
async function handleInvoiceDirectSync({ requestId, payload }) {
  console.log(`⚙️ Syncing invoice direct to Tally for request ${requestId}...`);
  try {
    const salesLedgerXml = generateLedgerXml("Sales", "Sales Accounts", "", "", "", "", "");
    await postToTally(salesLedgerXml);
    ...
    const xml = generateSalesVoucherXml(
      payload.invoiceNo,
      payload.date,
      payload.clientName,
    ...
    const response = await postToTally(xml);
    const isSuccess = response.includes("CREATED") || response.includes("ALTERED");
    
    ws.send(JSON.stringify({
      type: "sync:invoice:direct:result",
      data: { requestId, success: isSuccess, error: isSuccess ? null : "Tally import failed" }
    }));
  } catch (error) {
    ws.send(JSON.stringify({
      type: "sync:invoice:direct:result",
...
async function handleReceiptDirectSync({ requestId, payload }) {
  console.log(`⚙️ Syncing receipt directly to Tally for request ${requestId}...`);
  try {
    const bankXml = generateLedgerXml(payload.depositLedger, "Bank Accounts", "", "", "", "", "");
    await postToTally(bankXml);

    const xml = generateReceiptVoucherXml(
      payload.receiptNo,
      payload.paymentDate || payload.date,
      payload.clientName,
      payload.amount,
      payload.depositLedger
    );
    ...
    ws.send(JSON.stringify({
      type: "sync:receipt:direct:result",
      data: { requestId, success: isSuccess, error: isSuccess ? null : "Tally receipt import failed" }
    }));
  } catch (error) {
    ws.send(JSON.stringify({
      type: "sync:receipt:direct:result",
...
function generateReceiptVoucherXml(receiptNo, date, clientName, amount, depositLedger) {
  const formattedDate = formatDate(date);
  const escapedReceiptNo = escapeXml(receiptNo);
  const escapedClientName = escapeXml(clientName);
  const escapedDepositLedger = escapeXml(depositLedger);
  
  const clientAmt = Math.abs(amount);
  const bankAmt = -Math.abs(amount);
          ...
          <REQUESTDATA>
            <TALLYMESSAGE xmlns:UDF="TallyUDF">
              <VOUCHER VCHTYPE="Receipt" ACTION="Create">
                ...
                <VOUCHERNUMBER>${escapedReceiptNo}</VOUCHERNUMBER>
                <PARTYLEDGERNAME>${escapedClientName}</PARTYLEDGERNAME>
                <PARTYNAME>${escapedClientName}</PARTYNAME>
                <EFFECTIVEDATE>${formattedDate}</EFFECTIVEDATE>
                ...
                <ALLLEDGERENTRIES.LIST>
                  <LEDGERNAME>${escapedDepositLedger}</LEDGERNAME>
                  <ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>
...
// Dynamic pull for expenses (Payment Vouchers)
...
async function handleExpenseDirectSync({ requestId, payload }) {
  console.log(`⚙️ Syncing expense directly to Tally for request ${requestId}...`);
  try {
    const expenseXml = generateLedgerXml(payload.category, "Indirect Expenses", "", "", "", "", "");
    await postToTally(expenseXml);

    const bankXml = generateLedgerXml(payload.bankOrCashLedger, "Bank Accounts", "", "", "", "", "");
    await postToTally(bankXml);

    const xml = generatePaymentVoucherXml(
      payload.voucherNo,
      payload.date,
      payload.category,
      payload.amount,
      payload.payee || "Vendor / Staff",
      payload.bankOrCashLedger
    );
    ...
    ws.send(JSON.stringify({
      type: "sync:expense:direct:result",
      data: { requestId, success: isSuccess, error: isSuccess ? null : "Tally payment import failed" }
    }));
  } catch (error) {
    ws.send(JSON.stringify({
      type: "sync:expense:direct:result",
...
function generatePaymentVoucherXml(voucherNo, date, expenseLedger, amount, payee, bankOrCashLedger) {
  const formattedDate = formatDate(date);
  const escapedVoucherNo = escapeXml(voucherNo);
  const escapedExpenseLedger = escapeXml(expenseLedger);
  const escapedPayee = escapeXml(payee);
  const escapedBankOrCashLedger = escapeXml(bankOrCashLedger);
          ...
          <REQUESTDATA>
            <TALLYMESSAGE xmlns:UDF="TallyUDF">
              <VOUCHER VCHTYPE="Payment" ACTION="Create">
                ...
                <PARTYLEDGERNAME>${escapedBankOrCashLedger}</PARTYLEDGERNAME>
                <PARTYNAME>${escapedPayee}</PARTYNAME>
                <EFFECTIVEDATE>${formattedDate}</EFFECTIVEDATE>
                
                <ALLLEDGERENTRIES.LIST>
                  <LEDGERNAME>${escapedExpenseLedger}</LEDGERNAME>
                  <ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>
                  <AMOUNT>${expenseAmt}</AMOUNT>
                </ALLLEDGERENTRIES.LIST>
                
                <ALLLEDGERENTRIES.LIST>
                  <LEDGERNAME>${escapedBankOrCashLedger}</LEDGERNAME>
                  <ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>
                  <AMOUNT>${bankAmt}</AMOUNT>
                </ALLLEDGERENTRIES.LIST>
              </VOUCHER>
...
// Pull aging reports (Outstanding client receivables)
